...
def disasm_func(name, rva, max_lines=40):
    va = image_base + rva
    fo = va_to_fo(va)
    ...
    for insn in md.disasm(code, va):
        ...
        count += 1
        if insn.mnemonic in ['ret', 'jmp'] and count > 1:
            if insn.mnemonic == 'jmp' and 'qword ptr' not in insn.op_str:
                pass
            else:
                break
...
disasm_func('Bambu_Open', 0x4940)
disasm_func('Bambu_StartStream', 0x4990)
disasm_func('Bambu_ReadSample', 0x4950)
disasm_func('Bambu_GetStreamInfo', 0x4920)
